...
- When implementing UI under `ts/app`, always check the available reusable/base
  components in `ts/app/src/ui` first and use them where possible. If a
  component or pattern is used many times across the app, extract it or add it
  to `ts/app/src/ui` instead of duplicating the implementation.
...
- Dynamic user, business, and domain data displayed to the user MUST never be
  faked, stubbed, hardcoded, or mocked in product code. Metrics, entities,
  statuses, rows, and calculated values shown in a view must come from a real
  data source (API, database, store, or live computation). Do not ship
  placeholder numbers, dummy rows, lorem-ipsum content, or "TODO: wire up real
  data" values in screens users can reach. Static product copy, navigation
  labels, accessibility text, form placeholders, design constants, and explicit
  loading, empty, error, or unavailable-data states are not dynamic data and
  may be defined in product code. If the real source is not available yet,
  render an explicit unavailable, empty, loading, or error state instead of
  inventing data, and wire the view to the real source before the feature is
  considered complete. Faked or stubbed data is only acceptable in tests,
  mocks, fixtures, and mockups under `docs/mockups`.
- User-facing copy must read as product language written for the user, not
  ...
  both mockups under `docs/mockups` and product implementation (`ts/app`,
  `ts/web`, and other user-facing surfaces).
...
* Preview seed migrations are also immutable once committed or applied by a
  preview database. Do not rewrite, delete, renumber, or "clean up" an existing
  seed migration file; add a new forward seed migration for fixture changes or
  repairs.
...
* `*-store-pg` crates may depend on their store-interface crate and `fna-db` for shared schema/migrations, plus necessary external database/serialization/error crates
